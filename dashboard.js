...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadTasks() {
        ...
        tasks.forEach(function(task) {
            ...
            deleteButton2.classList.add('delete-btn'); // Add a class for styling
            deleteButton2.classList.add('delete-btn-t');
            deleteButton2.addEventListener('click', function() {
                taskList.removeChild(newTask);
                removeTaskFromStorage(task); // Remove task from localStorage
            });

            newTask.appendChild(deleteButton2);
            taskList.appendChild(newTask);
        });
    }
    ...
    records.forEach(record => {
        ...
        recordElement.innerHTML = `
            ...
            <div class="record-actions">
                <button class="delete-btn delete-btn-t2" data-id="${record.id}">Delete</button>
            </div>
        `;
